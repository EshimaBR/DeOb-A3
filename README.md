...
Uma ferramenta automatizada em lotes e PowerShell para corrigir o arquivo `config.cpp` e organizar os modelos 3D após a desofuscação de arquivos PBO do Arma 3 com o `DeMikeroObfuscator.exe`.
...
Depois de usar o seu `DeMikeroObfuscator.exe` em um arquivo PBO e ele gerar o `config.cpp` bugado mais aqueles arquivos `.p3d` genéricos, siga os passos a seguir:
